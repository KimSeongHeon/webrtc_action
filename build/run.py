...
def get_webrtc(source_dir, patch_dir, version, target,
    ...
    if not os.path.exists(os.path.join(webrtc_source_dir, 'src')):
        with cd(webrtc_source_dir):
            ...
            cmd(['git', 'clone', 'https://github.com/KimSeongHeon/webrtc_native.git', 'src'])
...
def main():
    ...
    bp.add_argument("--webrtc-fetch", action='store_false')
    bp.add_argument("--webrtc-fetch-force", action='store_false')
